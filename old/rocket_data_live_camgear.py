...
def get_rocket_data(arguments):
    ...
    labpadre = [0, 30, 1145, 1218]          # Position of clock in livestream (just for livestream testing)
    # Constants ######################################################################################################
    ...
    options = {"STREAM_RESOLUTION": "720p", "CAP_PROP_POS_MSEC": video_start_time * milliseconds}

    stream = CamGear(source=url, stream_mode=True, **options).start()

    full_url = "https://www.youtube.com/oembed?" + urllib.parse.urlencode({"format": "json", "url": url})
    ...
    while True and tf < video_duration:
