...
def get_rocket_data(arguments):
    ...
    upper_limit_alti_plot = 300             # Upper limit of altitude plot
    ...
    mean_of_last = 5                        # Mean value of last n acceleration values
    # General settings ###############################################################################################
    ...
    if video.author == "SpaceX":
        pos_stage = [f9_stage1, f9_stage2]
    elif video.author == "Rocket Lab":
        pos_stage = [rocketlab]
    else:
        pos_stage = None
        print("Youtube channel " + video.author + " not supported.")
        quit()
    ...
    while True and cap.get(cv2.CAP_PROP_POS_MSEC) <= video_end_time * 1000:
        ...
        for stage in range(1, number_of_stages + 1):
            ...
            v_frame, h_frame = get_text_from_frame(frame, pos_stage, stage)
...
def get_text_from_frame(frame, pos_stage, stage):
